...
View-change observation only; mouse/controller snaps are not proof of aiming assistance.

## Controls

Use `/ac-modstate aimbotmonitor on` or `/ac-modstate aimbotmonitor off`. The generic module handler requires clearance 3 or `paradox.modules`, plus clearance 4 or `paradox.settings` to change state. Operators have these permissions by default. Changes persist to SQLite and override TOML defaults. For utility commands and special cases, see [module controls](../commands/toggles.md) and [player utilities](../commands/utility.md).
