...
Rapid totem inventory-change review only; no punishment for pickup, lag or custom inventory tools.

## Controls

Use `/ac-modstate autototem on` or `/ac-modstate autototem off`. The generic module handler requires clearance 3 or `paradox.modules`, plus clearance 4 or `paradox.settings` to change state. Operators have these permissions by default. Changes persist to SQLite and override TOML defaults. For utility commands and special cases, see [module controls](../commands/toggles.md) and [player utilities](../commands/utility.md).
